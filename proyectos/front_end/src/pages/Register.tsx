...
const Register: React.FC = () => {
  ...
  // Lógica para evaluar la fuerza de la contraseña
  useEffect(() => {
    if (!password) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setStrength({ level: 0, label: '', color: 'bg-transparent' });
      return;
    }

    let score = 0;
    if (password.length >= 8) score += 1; // Longitud aceptable
    if (/[A-Z]/.test(password)) score += 1; // Tiene mayúsculas
    if (/[0-9]/.test(password)) score += 1; // Tiene números
    if (/[^A-Za-z0-9]/.test(password)) score += 1; // Tiene caracteres especiales

    if (password.length < 6) {
      setStrength({ level: 1, label: 'Débil', color: 'bg-red-500' });
    } else if (score < 3) {
      setStrength({ level: 2, label: 'Intermedio', color: 'bg-amber-500' });
    } else {
      setStrength({ level: 3, label: 'Fuerte', color: 'bg-emerald-500' });
    }
  }, [password]);
      ...
      <div className="flex w-full items-center justify-center bg-white px-8 lg:w-1/2">
        <div className="w-full max-w-md">
          <div className="mb-10 text-center lg:text-left">
            <h1 className="text-2xl font-light uppercase tracking-[0.2em] text-neutral-900 mb-2">Crear Cuenta</h1>
            <p className="text-xs uppercase tracking-widest text-neutral-500">Únete a nuestra Maison</p>
          </div>
              ...
              {password.length > 0 && (
                <div className="pt-2 animate-fadeIn">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[9px] uppercase tracking-widest text-neutral-500">Seguridad:</span>
                    <span className={`text-[9px] uppercase tracking-widest font-medium ${strength.color.replace('bg-', 'text-')}`}>
                      {strength.label}
                    </span>
                  </div>
                  <div className="flex h-1 w-full gap-1">
                    <div className={`h-full flex-1 rounded-full transition-colors duration-300 ${strength.level >= 1 ? strength.color : 'bg-neutral-100'}`}></div>
                    <div className={`h-full flex-1 rounded-full transition-colors duration-300 ${strength.level >= 2 ? strength.color : 'bg-neutral-100'}`}></div>
                    <div className={`h-full flex-1 rounded-full transition-colors duration-300 ${strength.level >= 3 ? strength.color : 'bg-neutral-100'}`}></div>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="mt-8 text-center lg:text-left">
            <Link to="/login" className="text-[10px] uppercase tracking-widest text-neutral-500 underline hover:text-neutral-900">
