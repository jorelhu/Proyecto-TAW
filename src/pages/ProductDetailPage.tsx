...
const ProductDetailPage = () => {
  ...
  const [activeAccordion, setActiveAccordion] = useState<string | null>("notes");  const [imageErrors, setImageErrors] = useState<boolean[]>(new Array(product.images.length).fill(false));
  const [imageLoading, setImageLoading] = useState<boolean[]>(new Array(product.images.length).fill(true));
          ...
          {imageErrors[activeImage] ? (
            ...
            <>
              ...
              <img
                ...
                alt={product.name}
                className={`max-h-full w-auto object-contain transition-opacity duration-500 ${imageLoading[activeImage] ? 'opacity-0' : 'opacity-100'}`}
